...
def makeChallengePolygon(n, inrad, outrad, pierce=0, red=0):
    ...
    for i in range(0, n + 1, 1):
        a = i * (math.pi * 0.5 / n)
        l.append(str(math.ceil(math.cos(a) * (inrad + outrad) / (2 + pierce))) + " " + str(
            math.ceil(math.sin(a) * (inrad + outrad) / (2 + pierce))))
...
def makeInputPolygon(filename, size, rndm=0):
    ...
    for p in pts:
        if (rndm == 0):
            ...
            l.append(str((p[1] - 1) * size + random.randint(0, size)) + " " + str(
                size * (len(inp) - 1) - (p[0] - 1) * size - random.randint(0, size)))
    # print(l)
    print("Done")
    return l
...
def writePoints(poly,
                output='filename'):  # Writes a list poly at [output]_#####.txt with # some random characters. Could technically overwrite an older one I guess, but not likely
